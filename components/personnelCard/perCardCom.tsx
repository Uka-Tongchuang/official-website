...
function perCardCom() {
  ...
  return (
    <div className="md:h-auto bg-gradient-to-b from-[#fff] to-[#FAFBFF]">
      ...
      {/* //卡片 */}
      <div className="card_box grid grid-cols-2 xs:grid-cols-3  gap-4  md:w-[80%] md:grid-cols-3 md:gap-8 p-4 w-full xl:w-[80%] m-auto xl:grid xl:grid-cols-5 xl:gap-7">
        {content.servers.cardarr.map((item, index) => (
          ...
          >
            <h4 className="font-bold text-md md:text-lg xl:text-lg">
              {item.title}
            </h4>
            <p className="text-sm text-ccc w-[60%] h-10 mt-10 md:text-md">
              {item.en}
            </p>
            <Image
